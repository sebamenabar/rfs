...
class ImageNet(Dataset):
    def __init__(
        ...
        if fname is None:
            ...
            self.file_pattern = fname
        self.data = {}
        with open(
            os.path.join(self.data_root, self.file_pattern % partition), "rb"
        ) as f:
...
class MetaImageNet(ImageNet):
    def __init__(
        ...
        if test_transform is None:
            self.test_transform = transforms.Compose(
                [lambda x: Image.fromarray(x), transforms.ToTensor(), self.normalize]
            )
        else:
    ...
    def __getitem__(self, item):
        ...
        for idx, cls in enumerate(cls_sampled):
            ...
            support_xs_ids_sampled = np.random.choice(
        ...
        if self.n_aug_support_samples > 1:
            ...
            support_ys = np.tile(
                support_ys.reshape((-1,)), (self.n_aug_support_samples)
            )
