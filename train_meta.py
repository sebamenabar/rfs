...
def main():
    ...
    for step in pbar:
        ...
        if (step == 1) or (step % opt.eval_freq == 0):
            ...
            print(f"{opt.n_ways}-way-{opt.n_shots}-shot accuracy: {val_acc.item()}")
        ...
        # time2 = time.time()
        # print("step {}, total time {:.2f}".format(step, time2 - time1))
        ...
        #         {
        #             "acc": test_acc.cpu(),
        #             "acc_top5": test_acc_top5.cpu(),
        #             "loss": test_loss,
        #         },
        #         step=step,
        #     )

        # logger.log_value("test_acc", test_acc, step)
        # logger.log_value("test_acc_top5", test_acc_top5, step)
        # logger.log_value("test_loss", test_loss, step)
        ...
        #         opt.save_folder, "ckpt_step_{step}.pth".format(step=step)
        #     )
...
def train_step(
    ...
    for task_num in range(x_spt.size(0)):
        ...
        with higher.innerloop_ctx(classifier, inner_opt) as (fmodel, diffopt):
